	...
	public static function conversionChecks(song:SongData):SongData
	{
		...
		for (section in song.notes)
		{
			for (notes in section.sectionNotes)
			{
				...
				if (notes[3] == null || notes[3] == 'true' || notes[3] == 'false' || notes[3] == 0 || notes[3] == 0.0 || Math.isNaN(notes[3]))
					notes[3] = 'normal';

				if (notes[4] == null || notes[4] == 0 || notes[4] == 0.0 || notes[4] == '0' || notes[4] == '0.0' || Math.isNaN(notes[4]))
					notes[4] = 1.0;
